...
def parsers_numbers():
    def content():
        def inner_content():  # <-- этот контент попадёт в твой контейнер
            ui.label('Парсеры Номеров').style('font-size: 32px; font-weight: bold; margin-bottom: 18px;')
            ...
            ui.label('Логи парсера').style('font-weight: bold; font-size: 20px; margin-bottom: 8px;')

            log_box = ui.markdown('```\n' + '\n'.join(logs) + '\n```') \
                .classes('bg-black text-white px-6 py-4 rounded-lg') \
                .style('font-size: 15px; width: 100%; max-width: 900px; font-family: "JetBrains Mono", monospace; white-space: pre-wrap; max-height: 300px; overflow-y: auto;')
